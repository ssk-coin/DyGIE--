...
class RelationModule(nn.Module):
    ...
    def __init__(
        ...
        self._span_proj_dim = span_proj_dim if span_proj_dim > 0 else None
        ...
        # span_proj_dim=0 のときは射影せず span_dim をそのまま使う。
        _proj = span_proj_dim if span_proj_dim > 0 else span_dim
        if span_proj_dim > 0:
            self.span_proj = nn.Sequential(
                nn.Linear(span_dim, _proj),
                nn.ReLU(),
                nn.Dropout(dropout),
            )
        else:
        ...
        pair_input_dim = _proj * 3 + type_feat_dim + dist_feat_dim
